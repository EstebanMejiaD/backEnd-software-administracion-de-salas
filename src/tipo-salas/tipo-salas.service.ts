...
export class TipoSalasService {
  ...
  async create(createTipoSalaDto: CreateTipoSalaDto, user: Usuario) {
    try {
      ...
      return {
        status: 201,
        msg: 'Tipo de sala creada satisfactoriamente',
        nuevoTipoSala,
      };
    } catch (error) {
  ...
  async findAll({ limit, offset }: PaginationTipoSalaDto) {
    try {
      const tiposSalas = await this.tipoSalaRepository.find({
        where: { estado: true },
        skip: offset,
        take: limit,
      });
      ...
      if (!tiposSalas) {
        return new NotFoundException({
          status: 404,
          msg: 'No se encontraron registros de tipos de salas',
        });
      }

      return {
        status: 200,
        msg: 'Tipos de salas obtenidos',
        tiposSalas,
      };
    } catch (error) {
      this.handleDBErrors(error);
  ...
  async findOne(id: string) {
    try {
      const tiposala: TipoSala = await this.tipoSalaRepository.findOneBy({
        id,
      });
      ...
      if (!tiposala) {
        return new NotFoundException({
          status: 404,
          msg: 'El tipo de sala que estas buscando, no existe',
        });
      }
  ...
  async update(id: string, updateTipoSalaDto: UpdateTipoSalaDto) {
    try {
      ...
      if (!tiposala) {
        return new NotFoundException({
          status: 404,
          msg: 'El tipo de sala no existe, no existe'
        });
      }
      ...
      return {
        status: 200,
        msg: 'Se ha actualizado el tipo de sala'
      };
    } catch (error) {
      this.handleDBErrors(error);
    }
  ...
  async actualizarEstado(id: string) {
    try {
      ...
      return {
        status: 200,
        msg: 'Se ha eliminadio el tipo de sala'
      };
    } catch (error) {
      this.handleDBErrors(error);
    }
